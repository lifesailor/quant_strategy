...
class DAE:
    def __init__(self, params, name=None):
        ...
        l1_beta = params['l1_beta']
        l2_beta = params['l2_beta']
        self.encoders = []
        self.auto_encoders = []
        for i in range(len(self.units)):
            self.encoders.append(tf.keras.layers.Dense(self.units[i], activation=self.activation, kernel_regularizer=tf.keras.regularizers.l1_l2(l1_beta, l2_beta), bias_regularizer = tf.keras.regularizers.l1_l2(l1_beta, l2_beta)))
        ...
        for i in range(len(self.units) - 1, -1, -1):
            self.auto_encoders.append(tf.keras.layers.Dense(self.units[i], activation=self.activation, kernel_regularizer=tf.keras.regularizers.l1_l2(l1_beta, l2_beta), bias_regularizer = tf.keras.regularizers.l1_l2(l1_beta, l2_beta)))
            if self.batch_norm[i]:
    ...
    def get_encoder_objects(self):
        return tf.keras.Sequential(self.encoders, name='DAE')
    ...
    def noise_injection(self, batch, all_data, noise_ratio=0.15):
        ...
        for i in range(batch_size):
            random_swap = np.random.sample(n_features) < noise_ratio
