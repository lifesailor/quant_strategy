...
for(statrun in 1:2){
  ...
  EPS <- as.data.frame(read.csv("./data/EPS-em.csv", row.names=1,header=TRUE))[,colnames(RET)]
  EPS1 <- as.data.frame(read.csv("./data/EPS1-em.csv", row.names=1,header=TRUE))[,colnames(RET)]
  index1 <- as.data.frame(read.csv("./data/priceindex-mon-em.csv", row.names=1,header=TRUE))[,colnames(RET)]
  minobs1=12
  minobs=60
...
# write csv
write.csv(functionoutput[[5]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\empe.csv')
write.csv(functionoutput[[6]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\empe_lag.csv')
