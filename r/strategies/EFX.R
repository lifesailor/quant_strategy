...
for(statrun in 1:2){
  ...
  fx<- as.data.frame(read.csv("./data/fx.csv", row.names=1,header=TRUE))[,colnames(RET)]
  ...
  # Expanding Moving Average of FX Reverse strenght
  ...
  # Signal 2. Revese of FX Strenght / 12 month STDEV
  RV=(-Mag)/STDEV1
...
# write csv
write.csv(functionoutput[[5]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\efx.csv')
write.csv(functionoutput[[6]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\efx_lag.csv')
