...
for(statrun in 1:2){
  ...
  roa <- as.data.frame(read.csv("data/roa.csv", row.names=1,header=TRUE))[,colnames(RET)]
  icr <- as.data.frame(read.csv("data/ICR.csv", row.names=1,header=TRUE))[,colnames(RET)]
...
# write csv
write.csv(functionoutput[[5]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\eql.csv')
write.csv(functionoutput[[6]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\eql_lag.csv')
