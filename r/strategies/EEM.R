...
for(statrun in 1:2){
  ...
  ##volupdate trigger
  Volband=0.1
  err<- as.data.frame(read.csv("./data/ERR.csv", row.names=1,header=TRUE))[,colnames(RET)]
  ...
  ##Reliability
  # 6 month average of (err - 12 month average err)
...
# write csv
write.csv(functionoutput[[5]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\eem.csv')
write.csv(functionoutput[[6]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\eem_lag.csv')
