...
carry=tenYR-twoYR
fx<- as.data.frame(read.csv("./data/fx.csv", row.names=1,header=TRUE))
carry=carry[rownames(carry)%in%rownames(fx),] # to monthly series
...
carry=carry[rownames(carry)%in%rownames(STDEV),]/STDEV # vol adjusted carry
...
Ret=RET
RV=carry[4:nrow(carry),]-carry[1:(nrow(carry)-3),] # 3 month change of carry
...
carry=carry[rownames(carry)%in%rownames(fx),]
Ret=RET
RV=carry[7:nrow(carry),]-carry[1:(nrow(carry)-6),] # 6 month change of carry
...
# write csv
write.csv(functionoutput[[5]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\ica2.csv')
write.csv(functionoutput[[6]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\ica2_lag.csv')
