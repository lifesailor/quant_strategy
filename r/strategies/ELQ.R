...
for(statrun in 1:2){
  ...
  fx<-as.data.frame(read.csv("./data/m2gdp.csv", row.names=1,header=TRUE))[,colnames(RET)]
  ...
  # 6 month return / 6 month stdev of m2gdp
  RV=(Mag)/STDEV1
  ...
  ####Final Position
  # only use TS2 since WGT2[1] is 0
...
# write csv
write.csv(functionoutput[[5]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\elq.csv')
write.csv(functionoutput[[6]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\elq_lag.csv')
