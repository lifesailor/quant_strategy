...
for(statrun in 1:2){
  ...
  DPS <- as.data.frame(read.csv("./data/DPS.csv", row.names=1,header=TRUE))[,colnames(RET)]
  DPS1 <- as.data.frame(read.csv("./data/DPS1.csv", row.names=1,header=TRUE))[,colnames(RET)]
...
# write csv
write.csv(functionoutput[[5]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\edy.csv')
write.csv(functionoutput[[6]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\edy_lag.csv')
