...
for(statrun in 1:2){
  ...
  yield <- as.data.frame(read.csv("./data/10Yield.csv", row.names=1,header=TRUE))[,colnames(RET)]
  EPS <- as.data.frame(read.csv("./data/EPS.csv", row.names=1,header=TRUE))[,colnames(RET)]
  EPS1 <- as.data.frame(read.csv("./data/EPS1.csv", row.names=1,header=TRUE))[,colnames(RET)]

  index1 <- as.data.frame(read.csv("./data/priceindex-mom.csv", row.names=1,header=TRUE))[,colnames(RET)]
  ...
  # expanding window rank Cross sectional.
  ...
  # signal 2. EPS / EPS1. what is 'EPS1.csv' ?? no idea..
  Rvalue=EY/EYD
  ...
  #
  CSRV=CSRV+CSRV6
  TSRV=TSRV#+TSRV6 # expclude signal1 for TS
...
# write csv
write.csv(functionoutput[[5]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\epe.csv')
write.csv(functionoutput[[6]], file='C:\\Users\\jungyoon.choi\\Desktop\\jungyoon\\project\\risk_premia\\python\\strategy\\check\\past\\epe_lag.csv')
